    ...
    return (
        <div className="py-3 navBarStyle">
            <div className="row w-100">
                ...
                <Dropdown isOpen={dropdown} toggle={abriCerrarDrop} className="col-sm col-12 text-center my-auto pb-2" >
                    ...
                    <Dropdown.Menu>
                        {unicos.map((product) => (
                            <Dropdown.Item id="dropItem">
                                <NavLink to={"category/" + product.id} className="nombresCategorias" onClick={(e) => { setCategoria(product.name) }}>{product.name}</NavLink>
                            </Dropdown.Item>
                        )
                        )}
                    </Dropdown.Menu>
